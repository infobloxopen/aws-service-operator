...
func NewOperator(config config.Config, queueManager *queuemanager.QueueManager) *Operator {
	...
	topicARN, _ := queuectrl.Register("vpc")
...
func (c *Operator) StartWatch(ctx context.Context, namespace string) {
	...
	oper := operator.New("vpcs", namespace, resourceHandlers, c.config.AWSClientset.RESTClient())
	oper.Watch(&awsV1alpha1.Vpc{}, ctx.Done())
}
...
func QueueUpdater(config config.Config, msg *queuemanager.MessageBody) error {
	...
	if msg.Updatable {
		...
		resources, err := clientSet.Vpcs(config.K8sNamespace).List(metav1.ListOptions{})
		if err != nil {
			logger.WithError(err).Error("error getting vpcs")
			return err
		}
...
func (c *Operator) onAdd(obj interface{}) {
	s := obj.(*awsV1alpha1.Vpc).DeepCopy()
	if s.Status.ResourceStatus == "" || s.Status.ResourceStatus == "DELETE_COMPLETE" {
		...
		if err != nil {
			c.config.Logger.WithError(err).Errorf("error creating vpc '%s'", s.Name)
			return
		}
		c.config.Logger.Infof("added vpc '%s' with stackID '%s'", s.Name, string(*output.StackId))
...
func (c *Operator) onUpdate(oldObj, newObj interface{}) {
	oo := oldObj.(*awsV1alpha1.Vpc).DeepCopy()
	no := newObj.(*awsV1alpha1.Vpc).DeepCopy()
	...
	if helpers.IsStackComplete(oo.Status.ResourceStatus, false) && !reflect.DeepEqual(oo.Spec, no.Spec) {
		...
		if err != nil {
			c.config.Logger.WithError(err).Errorf("error updating vpc '%s' with new params %+v and old %+v", no.Name, no, oo)
			return
		}
		c.config.Logger.Infof("updated vpc '%s' with params '%s'", no.Name, string(*output.StackId))
		c.config.Logger.Infof("view at https://console.aws.amazon.com/cloudformation/home?#/stack/detail?stackId=%s", string(*output.StackId))
...
func (c *Operator) onDelete(obj interface{}) {
	...
	err := cft.DeleteStack()
	if err != nil {
		c.config.Logger.WithError(err).Errorf("error deleting vpc '%s'", s.Name)
		return
	}

	c.config.Logger.Infof("deleted vpc '%s'", s.Name)
}
...
func updateStatus(config config.Config, name string, namespace string, stackID string, status string, reason string) (*awsV1alpha1.Vpc, error) {
	logger := config.Logger
	clientSet, _ := awsclient.NewForConfig(config.RESTConfig)
	resource, err := clientSet.Vpcs(namespace).Get(name, metav1.GetOptions{})
	if err != nil {
...
func deleteStack(config config.Config, name string, namespace string, stackID string) (*awsV1alpha1.Vpc, error) {
	logger := config.Logger
	clientSet, _ := awsclient.NewForConfig(config.RESTConfig)
	resource, err := clientSet.Vpcs(namespace).Get(name, metav1.GetOptions{})
	if err != nil {
...
func syncAdditionalResources(config config.Config, s *awsV1alpha1.Vpc) (err error) {
	clientSet, _ := awsclient.NewForConfig(config.RESTConfig)
	resource, err := clientSet.Vpcs(s.Namespace).Get(s.Name, metav1.GetOptions{})
	if err != nil {
		return err
	}
	resource = resource.DeepCopy()

	_, err = clientSet.Vpcs(s.Namespace).Update(resource)
	if err != nil {
		return err
	}
	return nil
